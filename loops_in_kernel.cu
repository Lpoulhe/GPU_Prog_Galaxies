...
__global__ void computeHistogramDD(unsigned int *d_histDD, float *ra_real, float *decl_real, int *d_NoofReal) 
{
    ...
    if (idx < NoofReal) {
        for (int j=0; j<NoofReal; j++){
            float theta_rad = angular_distance(ra_real[idx], decl_real[idx], ra_real[j], decl_real[j]);
...
__global__ void computeHistogramRR(unsigned int *d_histRR, float *ra_sim, float *decl_sim, int *d_NoofSim) 
{
    ...
    if (idx < NoofSim) {
        for (int j=0; j<NoofSim; j++){
            float theta_rad = angular_distance(ra_sim[idx], decl_sim[idx], ra_sim[j], decl_sim[j]);
            float theta = theta_rad * 180.0 / acos(-1.0);
            int bin = (int) (theta * binsperdegree);
                                    ...
                                    int *d_NoofSim, int *d_NoofReal) 
{
    ...
    if (idx < NoofReal) {
        for (int j=0; j<NoofSim; j++){
            float theta_rad = angular_distance(ra_real[idx], decl_real[idx], ra_sim[j], decl_sim[j]);
            float theta = theta_rad * 180.0 / acos(-1.0);
            int bin = (int) (theta * binsperdegree);
